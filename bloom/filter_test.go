...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := twhutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(twhutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := twhutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := twhutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
