...
func (cs *CoinSet) TotalValue() (value twhutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue twhutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue twhutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount twhutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue twhutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue twhutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue twhutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue twhutil.Amount, coins []Coin) (Coins, error) {
...
// twhutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() twhutil.Amount {
	return twhutil.Amount(c.txOut().Value)
}
