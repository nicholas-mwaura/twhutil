...
func TestNewTxFromBytes(t *testing.T) {
	...
	tx, err := twhutil.NewTxFromBytes(testTxBytes)
	if err != nil {
...
func TestTxErrors(t *testing.T) {
	...
	_, err = twhutil.NewTxFromBytes(shortBytes)
	if err != io.EOF {
...
func TestTxHasWitness(t *testing.T) {
	...
	tx.HasWitness()  // Should not fail (see nicholas-mwaura/twhd#1543)
	...
	for _, msgTxWithoutWitness := range Block100000.Transactions[1:] {
		txWithoutWitness := twhutil.NewTx(msgTxWithoutWitness)
